...
public class adisyon_17 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					adisyon_17 frame = new adisyon_17();
	...
	public adisyon_17() {
		...
		try {
	        ...
	        ResultSet rs =st.executeQuery("select musteri_no,tarih from tables where table_id='17' ");
		...
		try {														
	     ...
	     ResultSet rs =st.executeQuery("select * from siparis_detay where (masa_numarasi='17') AND (musteri_no='"+musteri_no+"') ");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(cevap==0)
				{
                  ...
                  try {
			        ...
			        if(kalan>0)
			        {
			           st.executeUpdate("Update tables SET  price='0' where table_id='17' ");
			        	
			        	
			        	st.executeUpdate("INSERT INTO orders(tarih,fiyat,masa_numarasi,musteri_no)VALUES('"+tarih+"','"+kalan+"','17','"+musteri_no+"')");
			        	st.executeUpdate("Delete from siparis_detay where musteri_no='"+musteri_no+"' ");
			        	musteri_no++;
			        	st.executeUpdate("Update tables SET  musteri_no='"+musteri_no+"' where table_id='17' ");
		...
		try {														
	     ...
	     ResultSet rs =st.executeQuery("select * from alinanlar where (masa_numarasi='17') AND (musteri_no='"+musteri_no+"') ");
