...
public class adisyon_14 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					adisyon_14 frame = new adisyon_14();
	...
	public adisyon_14() {
		...
		try {
	        ...
	        ResultSet rs =st.executeQuery("select musteri_no,tarih from tables where table_id='14' ");
		...
		try {														
	     ...
	     ResultSet rs =st.executeQuery("select * from siparis_detay where (masa_numarasi='14') AND (musteri_no='"+musteri_no+"') ");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(cevap==0)
				{
                  ...
                  try {
			        ...
			        if(kalan>0)
			        {
			           st.executeUpdate("Update tables SET  price='0' where table_id='14' ");
			        	
			        	
			        	st.executeUpdate("INSERT INTO orders(tarih,fiyat,masa_numarasi,musteri_no)VALUES('"+tarih+"','"+kalan+"','14','"+musteri_no+"')");
			        	st.executeUpdate("Delete from siparis_detay where musteri_no='"+musteri_no+"' ");
			        	musteri_no++;
			        	st.executeUpdate("Update tables SET  musteri_no='"+musteri_no+"' where table_id='14' ");
		...
		try {														
	     ...
	     ResultSet rs =st.executeQuery("select * from alinanlar where (masa_numarasi='14') AND (musteri_no='"+musteri_no+"') ");
